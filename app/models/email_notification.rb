class EmailNotification < Notification
  ...
  def alert(job)
    ActionMailer::Base.mail(:from => "cronut@harrys.com", :to => self.value, :subject => "Job \"#{job.name}\" didn't run", :body => "#{job.name} -  Last ran at: #{job.last_successful_time_str}").deliver!
    return job.next_scheduled_time.to_i.to_s
  end

  def early_alert(job)
    ActionMailer::Base.mail(:from => "cronut@harrys.com", :to => self.value, :subject => "Job \"#{job.name}\" ran too early", :body => "#{job.name} -  Ran at: #{now_date_str}, Scheduled for: #{job.next_scheduled_time_str}").deliver!
  end
